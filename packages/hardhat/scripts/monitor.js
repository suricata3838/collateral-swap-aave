...
const getReserveData = async(contract, reserveToken) => {
  ...
  const symble = reserveToken.symbol;
  const address = reserveToken.tokenAddress;
  console.log("address:", address);
  const reserveData = await contract.getReserveData(address);
  console.log("liquidityRate:", utils.formatEther(reserveData.liquidityRate));
  console.log("averageStableBorrowRate:", utils.formatEther(reserveData.averageStableBorrowRate));
  const list =  reserveData.slice(0, 9).map(x => utils.formatEther(x));
  console.log(symble, ":", list);
  return list;
}
...
const main = async () => {
  ...
  await handleReserveData(contract, allReserves);
};
